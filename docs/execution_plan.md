...
| **Phase 2: Pretraining** | Use the `v5litepod-64` to train the 1.12B model at ~200k+ tokens/sec. Use the `v4-32` to run continuous intermediate evals via `lm-eval`. | GCP TPU `v5litepod-64` + `v4-32` |
| **Phase 3: SFT** | Convert Orbax to Safetensors. Prepare SFT data. Run TRL SFTTrainer on the massive AMD GPU. | AMD MI300X (192GB VRAM) |
